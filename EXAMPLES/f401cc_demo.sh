...
generate_header "main.h" $opt 401cc -M\
    -D NO                     0\
       NONE                   NO\
       OFF                    NO\
       YES                    \(\!NO\)\
    ...
    -F "  /* By default the clock source of SysTick is AHB/8 = 12.5 MHz. */"\
    -F ""\
...
tag=R
generate_header "rcc.h" -l 401cc -p RCC -m rcc -f init_rcc\
    \
    -D $tag '(HCLK == 100)'\
       ""\
       PLLM_VAL  15\
       PLLN_VAL  "(HCLK * PLLM_VAL * 2 / 25)  /* PLLN: HSE25/PLLM*PLLN/2 = HCLK */"\
       PLLN_0    "((PLLN_VAL >> 0) & 1)"\
       PLLN_1    "((PLLN_VAL >> 1) & 1)"\
       PLLN_2    "((PLLN_VAL >> 2) & 1)"\
       PLLN_3    "((PLLN_VAL >> 3) & 1)"\
    ...
    -F "  /* F4: enable PWR clock and set voltage scaling */"\
    -F "  RCC->APB1ENR |= RCC_APB1ENR_PWREN;"\
    -F "  PWR->CR |= PWR_CR_VOS_1;"\
    -F "  /* F4: 2 wait states + prefetch + caches for 100MHz */"\
    -F "  FLASH->ACR = FLASH_ACR_LATENCY_2WS | FLASH_ACR_PRFTEN | FLASH_ACR_ICEN | FLASH_ACR_DCEN;"\
    -F "}"\
    -F ""\
    -F "__STATIC_FORCEINLINE void $func_name(void) {"\
    -F "  #if $tag"\
    -F "    /* Enable HSE (25 MHz crystal) and wait until ready */"\
    -F "    RCC->CR |= RCC_CR_HSEON;"\
    -F "    while (!(RCC->CR & RCC_CR_HSERDY)) {}"\
    -F ""\
    -F "    /* HSE=25MHz, PLLM=15, PLLN=120, PLLP=2 => 100MHz */"\
    -F "    RCC->PLLCFGR = ("\
    -F "      RCC_PLLCFGR_PLLSRC_HSE"\
    -F "    | RCC_PLLCFGR_PLLM_0 | RCC_PLLCFGR_PLLM_1 | RCC_PLLCFGR_PLLM_2 | RCC_PLLCFGR_PLLM_3"\
    -F "    | RCC_PLLCFGR_PLLN_3 | RCC_PLLCFGR_PLLN_4 | RCC_PLLCFGR_PLLN_5 | RCC_PLLCFGR_PLLN_6"\
    -F "    );"\
    -F ""\
    ...
    -F "    RCC->CFGR = RCC_CFGR_HPRE_DIV1 | RCC_CFGR_PPRE1_DIV2 | RCC_CFGR_PPRE2_DIV1 | RCC_CFGR_SW_PLL;"\
...
"${py_gen[@]}" -l 401cc -p GPIOA GPIOC -m gpio -f init_gpio\
    ...
    -D USE_ANALOG_MODE_FOR_ALL_PINS_BY_DEFAULT 1\
       ...
       ""\
        PA11_AF8_USART6_TX "PIN_AF(11, 8ULL)"\
    ...
    -H '  USART6_EN  * PIN_MODE(11, PIN_MODE_AF)       /* PA11 -- USART6 TX  */   \'\
    -H ')'\
    -H ""\
    ...
    -H "  USART6_EN * PA11_AF8_USART6_TX              \\"\
    -H ")"\
    -H ""\
    -H "#define GPIOC_MODE ( PIN_MODE(13, PIN_MODE_OUTPUT) /* PC13 -- LED */ )"\
    \
    $force_inline\
    --no-def\
    > gpio.h
...
create_file "stm32f401ccux_flash.ld" << 'EOF'
ENTRY(Reset_Handler)
...
SECTIONS
{
  ...
  ASSERT(_estack - _end >= _Min_Stack_Size,
    "RAM overflow: .data + .bss + stack exceeds 64 KB")
        ...
        <TargetArmAds>
          ...
          <Cads>
            ...
            <VariousControls>
              ...
              <Define>STM32F401xC DEBUG</Define>
              <Undefine></Undefine>
        ...
        <Group>
          ...
          <Files>
            ...
            <File>
              ...
              <FileType>2</FileType>
              <FilePath>..\MDK-ARM\startup_stm32f401xc.s</FilePath>
            </File>
            <File>
...
void OnProjectLoad (void) {
  ...
  Project.AddSvdFile ("$(ProjectDir)/STM32F401.svd");
...
create_file "stm32f401cc.jflash" << 'EOF'
  AppVersion =
  FileVersion = 2
[GENERAL]
...
__STATIC_FORCEINLINE void process_systick_event(void) {
  ...
  /* Increment uptime counter and toggle PC13 LED based on bit 9 state
   * When bit 9 of uptime is set: turn LED on (BSRR BS13)
   * When bit 9 of uptime is clear: turn LED off (BSRR BR13)
...
#    https://raw.githubusercontent.com/STMicroelectronics/cmsis_device_f4/master/Source/Templates/gcc/startup_stm32f401xc.s
#    https://raw.githubusercontent.com/STMicroelectronics/cmsis_device_f4/master/Source/Templates/arm/startup_stm32f401xc.s
#
...
#    https://raw.githubusercontent.com/STMicroelectronics/cmsis_device_f4/master/Include/stm32f401xc.h
#
...
#
#    https://raw.githubusercontent.com/cmsis-svd/cmsis-svd-data/refs/heads/main/data/STMicro/STM32F401.svd 
#    https://raw.githubusercontent.com/cmsis-svd/cmsis-svd-data/refs/heads/main/data/STMicro/STM32F401.svd
     

fname1=("system_stm32f4xx.c" "startup_stm32f401xc.s")
fname2=("system_stm32f4xx.h" "stm32f4xx.h" "stm32f401xc.h")
...
url3="${raw_github}cmsis-svd/cmsis-svd-data/refs/heads/main/data/STMicro/STM32F401.svd"
...
download_file "${url3}" "STM32F401.svd"
