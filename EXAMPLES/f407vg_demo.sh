...
generate_header "main.h" $opt 407vg -M\
    -D NO                     0\
       NONE                   NO\
       OFF                    NO\
       YES                    \(\!NO\)\
       ON                     YES\
       ""\
       HCLK                   "168  /* 48 to 168 (MHz) */"\
       ""\
       SYSTICK_CLOCK_SOURCE   "0    /* 0 = HCLK / 8; 1 = HCLK         */"\
       SYSTICK_ENABLE         YES\
       SYSTICK_IRQ_ENABLE     NO\
    \
    -H "#if HCLK < 48 || HCLK > 168"\
    -H "  #error \"HCLK must be between 48 and 168 MHz\""\
    -H \#endif\
    \
    $force_inline\
    --post-init $func_name\
...
tag=R
generate_header "rcc.h" -l 407vg -p RCC -m rcc -f init_rcc\
    \
    -D $tag '(HCLK == 168)'\
       ""\
       PLLM_VAL  8\
    ...
    -F "  /* F4: enable PWR clock and set voltage scaling for 168 MHz */"\
    -F "  RCC->APB1ENR |= RCC_APB1ENR_PWREN;"\
    -F "  PWR->CR |= PWR_CR_VOS;"\
    -F "  /* F4: 5 wait states + prefetch + caches for 168 MHz */"\
    -F "  FLASH->ACR = FLASH_ACR_LATENCY_5WS | FLASH_ACR_PRFTEN | FLASH_ACR_ICEN | FLASH_ACR_DCEN;"\
    -F "}"\
    -F ""\
    -F "__STATIC_FORCEINLINE void $func_name(void) {"\
    -F "  #if $tag"\
    ...
    -F ""\
    -F "    /* HSE=8MHz, PLLM=8, PLLN=336, PLLP=2 => 168 MHz */"\
    -F "    /* PLLQ=7 => USB clock = 48 MHz */"\
    -F "    RCC->PLLCFGR = ("\
    -F "      RCC_PLLCFGR_PLLSRC_HSE"\
    -F "    | RCC_PLLCFGR_PLLM_3"\
    -F "    | RCC_PLLCFGR_PLLN_4 | RCC_PLLCFGR_PLLN_6 | RCC_PLLCFGR_PLLN_8"\
    -F "    | RCC_PLLCFGR_PLLQ_0 | RCC_PLLCFGR_PLLQ_1 | RCC_PLLCFGR_PLLQ_2"\
    -F "    );"\
    -F ""\
...
generate_header "gpio.h" -l 407vg -p GPIOA GPIOD -m gpio -f init_gpio\
    ...
    -D USE_ANALOG_MODE_FOR_ALL_PINS_BY_DEFAULT 1\
       ...
       ""\
        ...
        GPIOD_MODER "(GPIO_MODE ^ (GPIOD_MODE))"\
       ""\
    ...
    -H ""\
    -H "#define GPIOD_MODE (                              \\"\
    -H '  !0 * PIN_MODE(12, PIN_MODE_OUTPUT) /* PD12 -- LD3  Green   */ | \'\
    -H '  !0 * PIN_MODE(13, PIN_MODE_OUTPUT) /* PD13 -- LD4  Orange  */ | \'\
    -H '  !0 * PIN_MODE(14, PIN_MODE_OUTPUT) /* PD14 -- LD5  Red     */ | \'\
    -H '  !0 * PIN_MODE(15, PIN_MODE_OUTPUT) /* PD15 -- LD6  Blue    */   \'\
    -H ')'\
    -H ""\
    -H "#define GPIOD_OTYPE ( 0 )"\
    -H ""\
    -H "#define GPIOD_OSPEED ( 0 )"\
    \
    $force_inline\
    --no-def
...
JLINK_FLAGS = -openprj./stm32f407vg.jflash -open$(BUILD_DIR)/$(TARGET).hex -hide -auto -exit -jflashlog./jflash.log
...
create_file "stm32f407vgtx_flash.ld" << 'EOF'
ENTRY(Reset_Handler)
...
SECTIONS
{
  ...
  ASSERT(_estack - _end >= _Min_Stack_Size,
    "RAM overflow: .data + .bss + stack exceeds 112 KB")
}
EOF
    ...
    <Target>
          ...
          <TargetOption>
            <TargetCommonOption>
              ...
              <Cpu>IRAM(0x20000000,0x0001C000) IRAM2(0x10000000,0x00010000) IROM(0x08000000,0x00100000) CPUTYPE("Cortex-M4") FPU CLOCK(168000000) ELITTLE</Cpu>
        ...
        <TargetArmAds>
          ...
          <Cads>
            ...
            <VariousControls>
              ...
              <Define>STM32F407xx DEBUG</Define>
              <Undefine></Undefine>
        ...
        <Group>
          ...
          <Files>
            ...
            <File>
              ...
              <FileType>2</FileType>
              <FilePath>..\MDK-ARM\startup_stm32f407xx.s</FilePath>
            </File>
            <File>
...
void OnProjectLoad (void) {
  ...
  Project.AddSvdFile ("$(ProjectDir)/STM32F407.svd");
...
create_file "stm32f407vg.jflash" << 'EOF'
  AppVersion =
  FileVersion = 2
[GENERAL]
...
[DEVICE]
  ActiveComp = 0
  AutoProgrBankSel = 0x00000001
  AutoProgrMode = 0x00000001
  BankCount = 1
  CPUClkFreq = 0x00000000
  DeviceName = "STM32F407VG"
  DeviceSelect = 1
  FlashSize = 0x00100000
  FixedBankSel = 0
  HaltOnReset = 0x00000001
  IsHDL = 0
  IsRAMBoot = 0
  PowerOnReset = 0
  PrgRAM2Flash = 0
  ...
  RestoreBankSel = 0
  RAMSize = 0x0001C000
  SectorCount = 0x00000017
  SectorInfo = {
    0x00000000, 0x00004000, 0x00000000
    0x00004000, 0x00004000, 0x00000000
    0x00008000, 0x00004000, 0x00000000
    0x0000C000, 0x00004000, 0x00000000
    0x00010000, 0x00010000, 0x00000000
    0x00020000, 0x00020000, 0x00000000
    0x00040000, 0x00020000, 0x00000000
    0x00060000, 0x00020000, 0x00000000
    0x00080000, 0x00020000, 0x00000000
    0x000A0000, 0x00020000, 0x00000000
    0x000C0000, 0x00020000, 0x00000000
    0x000E0000, 0x00020000, 0x00000000
    0x00100000, 0x00020000, 0x00000000
    0x00120000, 0x00020000, 0x00000000
    0x00140000, 0x00020000, 0x00000000
    0x00160000, 0x00020000, 0x00000000
    0x00180000, 0x00020000, 0x00000000
    0x001A0000, 0x00020000, 0x00000000
    0x001C0000, 0x00020000, 0x00000000
    0x001E0000, 0x00020000, 0x00000000
    0x00200000, 0x00020000, 0x00000000
    0x00220000, 0x00020000, 0x00000000
    0x00240000, 0x00020000, 0x00000000
  }
  TargetPower = 0x00000000
  TipStr = "Cortex-M4"
[FLASH]
  AlgoFile = ""
  AlgoLoad = 0x00000001
  ...
  AlgoType = 0x00000000
  ClrStop = 0x00000000
  CodeCacheSize = 0x00008000
  CodeDownloadAddr = 0x20000000
  CodeStartAddr = 0x20000000
  CodeV2 = 0x00000001
  CpuClkSpeed = 0x00697800
  CustomDelayRoutineAddr = 0xFFFFFFFF
  Erase = 0x00000001
  EraseAction = 0x00000000
  EraseChip = 0x00000000
  EraseBefore = 0x00000001
  EraseSector = 0x00000001
  FProgram = 0x00000001
  FileDownloadAddr = 0x08000000
  FileFormat = 1
  FillByte = 0xFF
  FillLevel = 1
  FillReq4 = 0x00000000
  FileStart = 0x08000000
  FileSize = 0x00000000
  FileUnitSize = 0x00000000
  Info1AreaSize = 0x00000000
  Info2AreaSize = 0x00000000
  MaxChipEraseTime = 0x0000EA60
  MaxChipWriteTime = 0x00007530
  MaxChipFindTime = 0x00000000
  MinProgTime = 0x00000019
  NoOfFreeChips = 0x00000000
  NoOfRanges = 0x00000001
  Offset = 0x00000000
  Override = 0x00000000
  PCodeBuffer = 0x00000000
  Preselect = 0x00000000
  ProgAction = 0x00000000
  ProgBuffer = 0x00000000
  ProgDownloadAddr = 0x20000000
  ProgStart = 0x08000000
  ProgType = 0x00000001
  RAMBlockAddr = 0x00000000
  RAMBlockSize = 0x00000000
  ...
  RunAfterReset = 1
  RunAfterProg = 0
  SegmentSize = 0x00000000
  SelectBank0 = 0x00000000
  STLink2PCode = 0x00000000
  SerialIFType = 0x00000001
  SupportUninit = 0x00000001
  UseCFile = 0
  UseUVOFile = 0
  UserProg1 = ""
  UserProg2 = ""
...
#    https://raw.githubusercontent.com/STMicroelectronics/cmsis_device_f4/master/Source/Templates/gcc/startup_stm32f407xx.s
#    https://raw.githubusercontent.com/STMicroelectronics/cmsis_device_f4/master/Source/Templates/arm/startup_stm32f407xx.s
#
...
#    https://raw.githubusercontent.com/STMicroelectronics/cmsis_device_f4/master/Include/stm32f407xx.h
#
...
#
#    https://raw.githubusercontent.com/cmsis-svd/cmsis-svd-data/refs/heads/main/data/STMicro/STM32F407.svd 
#    https://raw.githubusercontent.com/cmsis-svd/cmsis-svd-data/refs/heads/main/data/STMicro/STM32F407.svd
     

fname1=("system_stm32f4xx.c" "startup_stm32f407xx.s")
fname2=("system_stm32f4xx.h" "stm32f4xx.h" "stm32f407xx.h")
...
url3="${raw_github}cmsis-svd/cmsis-svd-data/refs/heads/main/data/STMicro/STM32F407.svd"
...
download_file "${url3}" "STM32F407.svd"
